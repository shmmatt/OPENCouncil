...
- **State source crawling**: Separate pipeline for NH state agency documents with configurable target paths, link patterns, and exclude patterns. 9 state sources registered: OPD (Planning/Zoning handbooks), DES (Env rules), DRA (budgeting/taxation), NHMA (public Legal Q&As only — paywalled content excluded), RSAs, Admin Rules (OLS), Supreme Court (municipal-only filtering required), Fire/Building Code, DOT (driveway/access rules). Admin panel shows statewide stats alongside town stats and has a "Register Source" button for adding new sources.
